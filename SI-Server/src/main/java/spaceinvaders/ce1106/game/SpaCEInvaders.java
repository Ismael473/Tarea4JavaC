...
public class SpaCEInvaders
{
    ...
    Entity[] entities = new Entity[133]; // entities holds the fixed entities. i. e. all except bullets.
    // Entity count: 55 Aliens, 1 Player, 4 Bunkers of 19 BunkerBlocks (4*19=76), 1 UFO.
    // entities[0,54]->aliens; entities[55]->Player; entities[56,131]->BunkerBlocks; entities[132]->UFO;
    int right_descend_limit = 198;
    int left_descend_limit = 0;
    int player_height = 255;  // This is a y coordinate value that the bottom of the player sprite touches.
    ...
    Integer current_moving_alien = 0;
    ...
    Integer bunker_blocks_offset = 56;
    ...
    public SpaCEInvaders()
    {
        ...
        // Add the ufo to entities.
        ufo = aliens_concrete_factory.CreateUFO();
        ufo.x = ufo_starting_position_x;
        ufo.y = ufo_starting_position_y;
        entities[132] = ufo;
    }
    ...
    private void AdvanceFrame()
    {
        ...
        // UFO logic.
        if (!ufo.dead) {ufo.Move(false);}
        if (UFOSpawnCondition()) {ReviveAndResetUFO();}
        if (ufo.x > right_descend_limit) {ufo.dead = true;}
    ...
    private void MoveAliens()
    {
        ...
        Alien alien_to_move = GetNextAlien();
    ...
    private Boolean UpdateCollisions(Bullet bullet)
    {
        for (Entity entity : entities)
        {
            if (!entity.dead && EntitiesAreColliding(entity, bullet))
            {
                entity.SetAsDead();
                bullet.SetAsDead();
    ...
    // This method increases alien_counter so that dead aliens are skipped.
    Alien GetNextAlien()
    {
        if (current_moving_alien > 54){return null;}
        if (!this.aliens[current_moving_alien].dead){return this.aliens[current_moving_alien];}
        current_moving_alien ++;   // Skip Alien at aliens[current_moving_alien]
        return GetNextAlien();
    }
...
//    }
    ...
    private void AdvanceFrameLoadNextRound()
    {
        ...
        bunker_blocks_offset = entities.length - bunker_blocks_count - 1;
    ...
    private void AdvanceFrameSpawnAliens()
    {
        ...
        System.out.println("alien[" + current_game_state_frame_count+ "] (x, y, sprite, dead) = (" + aliens[current_game_state_frame_count].x + ", " + aliens[current_game_state_frame_count].y + ", " + aliens[current_game_state_frame_count].sprite + ", " + aliens[current_game_state_frame_count].dead + ")");
    }
...
//    }
    ...
    private void ReviveAndResetUFO()
    {
        System.out.println("ReviveAndResetUFO()");
        ufo.dead = false;
        ufo.x = ufo_starting_position_x;
        ufo.y = ufo_starting_position_y;
    }
